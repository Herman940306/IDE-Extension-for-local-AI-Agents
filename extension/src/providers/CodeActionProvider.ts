...
export class AICodeActionProvider implements vscode.CodeActionProvider {
  ...
  public async provideCodeActions(
    ...
    if (context.diagnostics.length > 0) {
      actions.push(
        ...(await this._createDiagnosticFixActions(document, range, context)),
      );
    }
  ...
  private _createSecurityActions(
    ...
    const hasSecurityIssues = context.diagnostics.some(
      (d) =>
        ...
        async (progress, token) => {
          ...
          if (!response || !response.actions || response.actions.length === 0) {
            vscode.window.showInformationMessage(
              "No AI suggestions available for this code.",
            );
            return false;
          }
          ...
          const action = await this._showActionPreview(
            document,
            response.actions,
          );
  ...
  private _generatePreviewText(
    action: CodeActionResponse["actions"][0],
  ): string {
    let preview = "Changes:\n\n";
  ...
  private _getActionType(actionType: string): CodeActionRequest["action_type"] {
    ...
    if (
      actionType.includes("refactor") ||
      actionType.includes("extract") ||
      actionType.includes("simplify")
    )
      return "refactor";
    return "fix";
  }
