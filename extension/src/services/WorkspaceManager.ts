...
export class WorkspaceManager {
  ...
  private async initialize() {
    ...
    const savedWorkspaces = this.context.globalState.get<
    ...
    const savedStates = this.context.globalState.get<
  ...
  ): Promise<Partial<WorkspaceConfig> | null> {
  ...
  public async updateAgentSettings(
    ...
    await this.saveWorkspaces();
    await this.saveWorkspaceConfig(id, {
  ...
  public async showWorkspaceSwitcher() {
    ...
    const items = workspaces.map((ws) => ({
      ...
      detail:
        ws.description ||
        `Last accessed: ${new Date(ws.lastAccessed).toLocaleString()}`,
      workspace: ws,
    }));
