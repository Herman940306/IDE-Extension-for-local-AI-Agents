...
export class AccessibilityManager {
    ...
    constructor(private context: vscode.ExtensionContext) {
        this.outputChannel = vscode.window.createOutputChannel('Enterprise AI - Accessibility');
    ...
    private loadConfig(): AccessibilityConfig {
        const config = vscode.workspace.getConfiguration('enterpriseAI.accessibility');
    ...
    private detectSystemSettings(): void {
        ...
        const isAccessibilityModeEnabled = vscode.workspace
     ...
     */
    public async updateConfig(updates: Partial<AccessibilityConfig>): Promise<void> {
        const config = vscode.workspace.getConfiguration('enterpriseAI.accessibility');
    ...
    public async showSettings(): Promise<void> {
        const options = [
            ...
            },
            {
                ...
                description: this.config.keyboardNavigationEnabled ? 'Enabled' : 'Disabled',
        ...
        const selected = await this.showAccessibleQuickPick(options, {
            ...
            ariaLabel: 'Accessibility settings menu. Use arrow keys to navigate, Enter to select.'
        });
    ...
    private async toggleSetting(setting: string): Promise<void> {
        switch (setting) {
            case 'screenReader':
                await this.updateConfig({ screenReaderEnabled: !this.config.screenReaderEnabled });
                break;
            case 'highContrast':
                await this.updateConfig({ highContrastMode: !this.config.highContrastMode });
                break;
            case 'fontSize':
                await this.adjustFontSize();
                break;
            case 'keyboardNav':
                await this.updateConfig({ keyboardNavigationEnabled: !this.config.keyboardNavigationEnabled });
                break;
            case 'announceActions':
                await this.updateConfig({ announceActions: !this.config.announceActions });
                break;
            case 'reducedMotion':
