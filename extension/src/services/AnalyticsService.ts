...
export class AnalyticsService {
  ...
  public getAgentMetrics(): AgentMetrics[] {
    const cached = this.getFromCache<AgentMetrics[]>(
      this.CACHE_KEYS.AGENT_METRICS,
    );
    if (cached) {
      return cached;
    }
  ...
  public getProductivityMetrics(): ProductivityMetrics {
    ...
    for (const event of this.events) {
      // By language
      if (event.language) {
        suggestionsByLanguage[event.language] =
          (suggestionsByLanguage[event.language] || 0) + 1;
      }
  ...
  public analyzeWorkflowPatterns(): WorkflowPattern[] {
    ...
    if (rates.acceptanceRate < 0.5 && rates.total > 10) {
      patterns.push({
        ...
        suggestion:
          "Consider adjusting AI settings or providing more context for better suggestions",
      });
    }

    // Pattern 2: Specific language dominance
    const metrics = this.getProductivityMetrics();
    const topLanguage = Object.entries(metrics.suggestionsByLanguage).sort(
      (a, b) => b[1] - a[1],
    )[0];
   ...
   */
  public getLanguageDistribution(): {
    language: string;
    count: number;
    percentage: number;
  }[] {
   ...
   */
  private loadEvents() {
    const stored = this.context.globalState.get<SuggestionEvent[]>(
      "analyticsEvents",
      [],
    );
    this.events = stored;
