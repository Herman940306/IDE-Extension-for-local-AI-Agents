...
export class ModeToggle {
  ...
  constructor(
    ...
    defaultMode: OperationMode = OperationMode.OFFLINE,
  ) {
    ...
    context.subscriptions.push(
      vscode.commands.registerCommand("enterpriseAI.toggleMode", () =>
        this.toggleMode(),
      ),
    );
   ...
   */
  public onModeChange(
    callback: (event: ModeChangeEvent) => void,
  ): vscode.Disposable {
  ...
  private updateStatusBar(): void {
    if (this.isOffline()) {
      ...
      this.statusBarItem.tooltip =
        "Offline Mode: All operations run locally (Click to switch to Online)";
      ...
      this.statusBarItem.tooltip =
        "Online Mode: Cloud LLM fallback enabled (Click to switch to Offline)";
      this.statusBarItem.backgroundColor = undefined;
  ...
  private _loadPersistedMode(): OperationMode | undefined {
    const persistedMode = this.context.workspaceState.get<string>(
      "enterpriseAI.operationMode",
    );

    if (
      persistedMode === OperationMode.OFFLINE ||
      persistedMode === OperationMode.ONLINE
    ) {
