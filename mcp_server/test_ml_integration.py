...
def test_ml_system_status():
    ...
    try:
        response = requests.get(f"{API_BASE}/ai/intelligence/status", timeout=5)
...
def test_command_reasoning():
    ...
    test_commands = ["turn on bedroom light", "make it cozy", "movie night setup"]

    for command in test_commands:
        try:
            response = requests.get(f"{API_BASE}/entities/test/{command}", timeout=5)
            response.raise_for_status()
            data = response.json()
...
def main():
    ...
    print("=" * 60)
    print(f"Results: {passed}/{total} tests passed ({passed / total * 100:.0%})")
    print("=" * 60)
