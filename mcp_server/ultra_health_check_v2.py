...
def check_ai_health():
    try:
        ...
        if r.status_code == 200:
            data = r.json()
            print(f"[✅] AI System is OPERATIONAL")
            features = data.get('features', {})
            print(f"  - Natural Language: {features.get('natural_language', False)}")
            print(f"  - Command Processing: {features.get('command_processing', False)}")
            print(f"  - Emotion Detection: {features.get('emotion_detection', False)}")
            print(f"  - Learning: {features.get('learning', False)}")
            print(f"  - WebSocket Support: {features.get('websocket_support', False)}")
            return True
        else:
...
def test_command(command_data):
    try:
        ...
        if r.status_code == 200:
            resp_json = r.json()
            print(f"[✅] Command '{command_data['text']}' executed successfully")
            message = resp_json.get('message', resp_json.get('response', 'No response'))
            print(f"  - Response: {message[:100]}...")
            return resp_json
        else:
            print(f"[❌] Command '{command_data['text']}' failed: HTTP {r.status_code}")
            return None
    except Exception as e:
        print(f"[❌] Exception executing command '{command_data['text']}': {e}")
        return None
