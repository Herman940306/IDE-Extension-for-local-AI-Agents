...
TEST_TOOL_ENDPOINT = f"{MCP_HOST}/command"
DASHBOARD_ENDPOINT = f"{MCP_HOST}/ai/health"
CACHE_ENDPOINT = f"{MCP_HOST}/health"  # ULTRA prefetch cache status
TIMEOUT = 5  # seconds
...
def call_tool(tool_call):
    try:
        ...
        r = requests.post(TEST_TOOL_ENDPOINT, data=payload, headers={"Content-Type": "application/json"}, timeout=TIMEOUT)
        if r.status_code == 200:
            resp_json = r.json()
            print(f"[✅] Tool '{tool_call['tool']}' executed successfully.")
            # Check ULTRA Master Prompt influence
            if resp_json.get("ultra_metadata"):
                print("  - ULTRA Master Prompt active, semantic & RLHF info detected")
            return resp_json
        else:
            print(f"[❌] Tool '{tool_call['tool']}' failed: HTTP {r.status_code}")
            return None
    except Exception as e:
...
def check_ultra_metrics():
    try:
        ...
        if r.status_code == 200:
            ...
            print(f"[❌] Failed to retrieve ULTRA metrics: HTTP {r.status_code}")
            return None
    except Exception as e:
...
def check_prefetch_cache():
    try:
        ...
        if r.status_code == 200:
            ...
            print(f"[❌] Failed to retrieve prefetch cache status: HTTP {r.status_code}")
            return None
    except Exception as e:
...
def calculate_ultra_confidence(metrics, cache):
    ...
    if metrics:
        ...
        score += (1 if metrics.get("auto_adaptation_events", 0) > 0 else 0) * 0.2
    if cache:
        score += min(cache.get("prefetch_hits", 0) / 10, 0.2)  # max 0.2 points from prefetch
