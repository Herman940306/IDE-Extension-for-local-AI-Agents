...
B904_FIXES = {
    "backend/src/adapters/crewai_adapter.py": [
        (88, "raise Exception(f\"Failed to initialize CrewAI adapter: {e}\")", 
         "raise Exception(f\"Failed to initialize CrewAI adapter: {e}\") from e"),
    ],
    "backend/src/adapters/superagi_adapter.py": [
        (67, "raise AdapterExceptions.AdapterConnectionError(", 
         "raise AdapterExceptions.AdapterConnectionError(", " from e"),
        (71, "raise AdapterExceptions.AdapterInitializationError(", 
         "raise AdapterExceptions.AdapterInitializationError(", " from e"),
        (250, "raise AdapterExceptions.AdapterConnectionError(", 
         "raise AdapterExceptions.AdapterConnectionError(", " from e"),
    ],
    ...
    "backend/src/services/llm_manager.py": [
        (115, "raise Exception(\"Ollama unavailable and cloud fallback disabled\")",
         "raise Exception(\"Ollama unavailable and cloud fallback disabled\") from None"),
        (125, "raise Exception(f\"Cannot connect to Ollama at {self.base_url}: {e}\")",
         "raise Exception(f\"Cannot connect to Ollama at {self.base_url}: {e}\") from e"),
        (413, "raise Exception(f\"LLM streaming failed: {e}\")",
...
if __name__ == '__main__':
    ...
    result = subprocess.run(
        ['.venv/Scripts/python.exe', '-m', 'ruff', 'check', 'backend', 
         '--select', 'E501,B017,B904', '--statistics'],
        capture_output=True, text=True
    )
