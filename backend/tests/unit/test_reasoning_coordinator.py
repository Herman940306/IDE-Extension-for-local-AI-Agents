...
def mock_reasoner():
    ...
    reasoner.reason = AsyncMock(return_value=ReasoningResponse(
    ...
    reasoner.get_stats = Mock(return_value={
        "model": "llama3.2:3b",
        "total_requests": 10,
...
def mock_verifier():
    ...
    verifier.verify = AsyncMock(return_value=VerificationResponse(
        valid=True,
        confidence=0.92,
        issues=[],
    ...
    verifier.get_stats = Mock(return_value={
        "model": "mistral:7b",
...
@pytest.mark.asyncio
async def test_adaptive_mode_high_confidence_skip_verification(coordinator, mock_reasoner, mock_verifier):
...
async def test_strategy_determination(coordinator):
    ...
    assert strategy == ProcessingMode.DUAL_PROCESS
