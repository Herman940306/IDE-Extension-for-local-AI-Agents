...
from models import CachePrediction
            ...
            if last_access in self.co_access:
                for related_id, count in self.co_access[last_access].items():
                    ...
                    if confidence >= self.prediction_threshold:
                        predictions.append(CachePrediction(
                            key=related_id,
                            predicted_access_time=datetime.fromtimestamp(current_time + 5),  # Soon
                            confidence=confidence,
        ...
        for prediction in predictions:
            try:
                ...
                if resource_type in self.preload_callbacks:
                    ...
                    logger.debug(f"Preloaded: {prediction.key} (confidence: {prediction.confidence:.2f})")
                else:
                    results["skipped"] += 1
