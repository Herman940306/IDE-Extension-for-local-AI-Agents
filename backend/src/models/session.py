...
class TaskContextPayload(BaseModel):
    ...
    language: Optional[str] = Field(default=None, description="Preferred programming language")
    file_path: Optional[str] = Field(default=None, description="Associated file path")
    repository: Optional[str] = Field(default=None, description="Repository or project identifier")
...
class TaskRequestPayload(BaseModel):
    ...
    content: Optional[str] = Field(default=None, description="Primary code or text content")
    context: TaskContextPayload = Field(
        default_factory=TaskContextPayload, description="Editor context supplied by the client"
    )
    priority: Priority = Field(default=Priority.MEDIUM, description="Desired task priority")
...
class TaskAcceptedPayload(BaseModel):
    ...
    received_at: float = Field(default_factory=lambda: time.time(), description="Server timestamp")
    message: str = Field(
...
class AgentRunResult(BaseModel):
    ...
    duration_ms: float = Field(..., ge=0.0, description="Processing latency in milliseconds")
