...
from ..models.reasoner import FastReasoner, ReasoningRequest, ReasoningResponse
from ..models.verifier import AnalyticalVerifier, VerificationRequest, VerificationResponse
...
class ReasoningCoordinator:
    ...
    def _determine_strategy(
        ...
        # Simple tasks → System 1 only
        if complexity < self.complexity_threshold:
            return ProcessingMode.SYSTEM1_ONLY
        
        # Complex tasks requiring verification → Dual process
        if task_analysis.get("requires_verification", False):
            return ProcessingMode.DUAL_PROCESS
        
        # Default to dual process for safety
        return ProcessingMode.DUAL_PROCESS
    ...
    async def _process_adaptive(
        ...
        should_verify = (
            system1_response.confidence < self.confidence_threshold or
            complexity > self.complexity_threshold or
            not system1_response.suggestions
        )
