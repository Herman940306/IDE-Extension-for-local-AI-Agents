...
class PersonaEngine:
    ...
    def blend_personas(self, names: List[str], weights: Optional[List[float]] = None) -> Dict[str, Any]:
        """
            ...
            if all(isinstance(v, dict) for v in vals if v is not None):
                ...
                for sk in subkeys:
                    ...
                    if all(isinstance(e, (int,float)) for e in entries if e is not None):
                        # weighted avg
                        total = sum((e or 0.0) * w for e,w in zip(entries, weights))
                        merged[sk] = total
                    else:
                ...
                chosen = None
                for val, w in sorted(zip(vals, weights), key=lambda x: -x[1]):
                    if val:
                        chosen = val
                        break
                blended[k] = chosen
    ...
    def select_emoji(self, sentiment: str = "neutral", intensity: float = 0.5, count: int = 1) -> List[str]:
        """
    ...
    def stylize_text(self, base_text: str, persona_name: Optional[str] = None,
                     sentiment: str = "neutral", empathy: float = 0.5, use_emoji: bool = True) -> Dict[str, Any]:
        """
        ...
        micro = random.choice(persona.get("micro_phrases", ["Got it.", "Understood.", "Right away."]))
        ...
        signature = random.choice(persona.get("signoffs", ["— AuralA", "— Your AuralA"])) if persona.get("signoffs") else ""
    ...
    def few_shot_system_prompt(self, persona_name: Optional[str] = None, archetype_weights: Optional[Dict[str,float]] = None) -> str:
        """
        ...
        if archetype_weights:
            ...
            blended = self.blend_personas(list(archetype_weights.keys()), list(archetype_weights.values()))
        else:
            blended = self.get_persona(persona_name)
        # Safety-first system prompt
        rules = blended.get("safety_rules", ["Be transparent that you are an AI assistant and avoid simulating human relationships."])
        ...
        prompt_lines.append("When replying, keep answers helpful, concise, and emotionally aware; use emojis sparingly to support tone.")
        return "\\n".join(prompt_lines)
    ...
    def prepare_llm_input(self, user_message: str, persona_name: Optional[str] = None,
                          sentiment: str = "neutral", empathy: float = 0.5, archetype_weights: Optional[Dict[str,float]] = None) -> Dict[str,Any]:
