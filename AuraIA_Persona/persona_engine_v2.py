...
class EmojiSelector:
    def __init__(self, emoji_file: Path = EMOJI_FILE):
        ...
        self.sentiment_map = {
            "neutral": [i for i,e in enumerate(self.emojis) if any(ch in e for ch in ["🙂","🤖","✨","💡"])],
            "friendly": [i for i,e in enumerate(self.emojis) if any(ch in e for ch in ["😊","🤝","🌟","🙌"])],
            "motivational": [i for i,e in enumerate(self.emojis) if any(ch in e for ch in ["🚀","🔥","💪","🏆"])],
            "thoughtful": [i for i,e in enumerate(self.emojis) if any(ch in e for ch in ["🤔","💭","🧠","🔍"])],
            "calm": [i for i,e in enumerate(self.emojis) if any(ch in e for ch in ["🌿","💧","🌙","🕊️"])],
            "error": [i for i,e in enumerate(self.emojis) if any(ch in e for ch in ["⚠️","🛑","🔧","😓"])],
            "success": [i for i,e in enumerate(self.emojis) if any(ch in e for ch in ["✅","🎉","🏅","🌟"])],
        }
    ...
    def pick(self, sentiment: str = "neutral", intensity: float = 0.5, username: Optional[str] = None, count: int = 1) -> List[str]:
        """
